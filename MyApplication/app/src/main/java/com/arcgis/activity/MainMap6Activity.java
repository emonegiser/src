...
//储备用地使用
public class MainMap6Activity extends Activity implements DrawEventListener,
    ...
    static MainMap6Activity instance;
    ...
    List<String> list_BPPC=new ArrayList<String>();
    List<String> list_CUN=new ArrayList<String>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        instance=MainMap6Activity.this;
        ...
        if(isNetwork){
                ...
                if(bppcStr!=null && !bppcStr.isEmpty()){
                    ...
                    for(int k=0;k<jsonArray.length();k++){
                        ...
                        if(o.has("PC_NAME") && o.get("PC_NAME")!=null){
                            ...
                            dzsysdicEntity.setType("CBYDBPPC");
                    ...
                    ToastUtil.show(MainMap6Activity.this,"服务器无返回值");
                }
            ...
            //获取储备用地村信息
            GetCBYDCUNDataSync getCBYDCUNDataSync=new GetCBYDCUNDataSync();
            try {
                String bppcStr=getCBYDCUNDataSync.execute().get(300, TimeUnit.SECONDS);
                if(bppcStr!=null && !bppcStr.isEmpty()){
                    list_CUN.clear();
                    JSONArray jsonArray = new JSONArray(bppcStr);
                    for(int k=0;k<jsonArray.length();k++){
                        ...
                        if(o.has("cun") && o.get("cun")!=null){
                            list_CUN.add(o.get("cun").toString());

                            dzsysdicEntity.setName(o.get("cun").toString());
                            dzsysdicEntity.setType("CBYDCUN");

                            if(!dzsysdicEntityDao.isEntityExist(o.get("cun").toString())){
                                dzsysdicEntityDao.add(dzsysdicEntity);
                            }
                        }
                    }
            ...
            list_BPPC.clear();
            List<DZSYSDICEntity> CBYDBPPCList =dzsysdicEntityDao.queryByType("CBYDBPPC");
            for(DZSYSDICEntity dz:CBYDBPPCList){
                list_BPPC.add(dz.getName());
            }

            list_CUN.clear();
            List<DZSYSDICEntity> CBYDCUNList =dzsysdicEntityDao.queryByType("CBYDCUN");
            for(DZSYSDICEntity dz:CBYDCUNList){
                list_CUN.add(dz.getName());
            }
        }
        ...
        if(list_CUN!=null && list_CUN.size()>0){
            list_CUN.add(0,"全部");
            MyApp.setCBYDCUN_List(null);
            MyApp.setCBYDCUN_List(list_CUN);
        }
        if(MyApp.getmGPSLon()!=null &&!MyApp.getmGPSLon().equals("")&&MyApp.getmGPSLat()!=null&&!MyApp.getmGPSLat().equals(""))
        {
            ToastUtil.show( MainMap6Activity.this,"定位当前位置 经度："+MyApp.getmGPSLon()+"  纬度："+MyApp.getmGPSLat());
            SharedPreferences sp = MainMap6Activity.this.getSharedPreferences("LOGIN_INFO",  MainMap6Activity.this.MODE_PRIVATE);
    ...
    public static MainMap6Activity getInstance(){
        if(instance!=null){
            return instance;
        }
        return null;
...
//        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (resultCode) {
            ...
            case RESULT_OK:
                if(requestCode== MainMap6Activity.TOADDPAGE){
    ...
    public void onClick(View v) {
        ...
        switch (viewID){
            case R.id.backtextview:
                MainMap6Activity.this.finish();
                break;

            case R.id.querypointTextView2:
                Intent intent=new Intent();
                intent.setClass(this,CBYDQueryActivity.class);
                startActivity(intent);
                break;
            case R.id.clearTextView2:
                AlertDialog.Builder builder = new AlertDialog.Builder(MainMap6Activity.this);
                ...
                builder.setPositiveButton("删除", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        MainMap6Activity.this.drawLayer.removeAll();
                        MainMap6Activity.this.drawTool.deactivate();
                    }
                });
    ...
    public class GetBPPCDataSync extends AsyncTask<String, Integer, String> {
        ...
        public GetBPPCDataSync() {
            ksoap=new KsoapValidateHttp(MainMap6Activity.this);
        }
